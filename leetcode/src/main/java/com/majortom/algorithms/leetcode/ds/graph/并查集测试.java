...
public class 并查集测试 {
  public static void main(String[] args) {
    Graph<Integer> graph = createGraph(6,
        new Integer[][] {{0, 4}, {1, 1}, {1, 5}, {2, 2}, {2, 3}, {2, 5}, {3, 2}, {3, 3}, {4, 0},
            {4, 4}, {5, 1}, {5, 2}, {5, 5}});
  ...
  private static boolean hasCycle(Graph<Integer> graph, Vertex<Integer> vertex,
      Vertex<Integer> parent, Set<Vertex<Integer>> visited) {
    visited.add(vertex);
    for (Vertex<Integer> neighbor : graph.neighbors(vertex)) {
      if (neighbor == vertex) {
        return true;
