package com.majortom.algorithms.leetcode.ds.matrix;

public class 对称矩阵 {
  public static void main(String[] args) {
    Integer[][] data = {{5, 3, 8, 6}, {3, 12, 9, 15}, {8, 9, 3, -1}, {6, 15, -1, 2}};
    int n = data.length;
  ...
  public static Integer[] zip(Integer[][] array) {
    ...
    Integer[] onearr = new Integer[n * (n + 1) / 2
        + 1]; // 一维数组保存下三角元素1+2+3+..+n，再预留一个空间，保留原始数据的阶数
    for (int i = 0; i < n; i++) {
      for (int j = 0; j <= i; j++) {
