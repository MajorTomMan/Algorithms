...
public class TreeController<T extends Comparable<T>> extends BaseModuleController<BaseTree<T>> {
    ...
    protected void setupI18n() {
        if (inputLabel != null)
            inputLabel.textProperty().bind(I18N.createStringBinding("label.tree.value"));
        if (insertBtn != null)
            insertBtn.textProperty().bind(I18N.createStringBinding("action.tree.insert"));
        if (randomBtn != null)
            randomBtn.textProperty().bind(I18N.createStringBinding("action.tree.random"));
        if (deleteBtn != null)
            deleteBtn.textProperty().bind(I18N.createStringBinding("action.tree.delete"));
        if (inputField != null)
            inputField.promptTextProperty().bind(I18N.createStringBinding("prompt.tree.value"));
    }
