 ...
 * 这里使用显式注册，保持 JavaFX 桌面应用的启动行为稳定，不做 classpath 扫描。</p>
 *
 * <p>这个注册表是“算法实现”和“界面选择器”的中间层。新增算法时，通常只需要：
 * 实现算法类、补充国际化文案、在这里注册 descriptor。控制器会按当前模块和结构类型自动拿到候选算法。</p>
 */
public final class AlgorithmRegistry {
     ...
     * 查询匹配模块、用途和结构类型的算法。
     *
     * <p>这三个条件共同保证 UI 不会展示不兼容的算法。例如迷宫模块切到图结构时，
     * 二维数组迷宫生成器不会被误放到图迷宫下拉框中。</p>
     *
     ...
     * 创建算法注册项。
     *
     * <p>这是内部小工厂，统一处理泛型擦除带来的类型噪声，让默认注册列表保持可读。</p>
     */
