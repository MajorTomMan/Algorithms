...
public record
    GridMaze(int rows, int columns, List<Boolean> openCells, GridPoint entrance, GridPoint exit) {
  public GridMaze {
    if (rows < 1 || columns < 1) {
    ...
    entrance =
        requireInside(Objects.requireNonNull(entrance, "entrance"), rows, columns, "entrance");
