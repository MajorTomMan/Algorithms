...
public final class AVLTree<T extends Comparable<? super T>> implements AvlTreeStructure<T> {
  ...
  private static <T extends Comparable<? super T>> Validation validateRestored(AVLTreeNode<T> node,
      T lowerExclusive, T upperExclusive, Set<AVLTreeNode<T>> identities, Set<Long> nodeIds) {
    if (node == null) {
    ...
    @SuppressWarnings("unchecked") AVLTreeNode<T> left = (AVLTreeNode<T>) node.getLeft();
    @SuppressWarnings("unchecked") AVLTreeNode<T> right = (AVLTreeNode<T>) node.getRight();
    ...
    if (node.getSubTreeCount() != expectedCount) {
      throw new IllegalArgumentException("restored AVL subtree count mismatch at node "
          + node.getId() + ": expected " + expectedCount + ", actual " + node.getSubTreeCount());
    }
  ...
  private void update(AVLTreeNode<T> node) {
    ...
    node.updateMetrics(Math.max(height(left(node)), height(right(node))) + 1,
