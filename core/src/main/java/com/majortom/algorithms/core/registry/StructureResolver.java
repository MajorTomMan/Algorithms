...
public final class StructureResolver {
  ...
  public StructureDescriptor resolve(Class<?> requiredContract, String structureId) {
    ...
    return candidates.stream()
        ...
        .findFirst()
        .orElseThrow(()
                         -> new RegistrationException("No Structure registered for contract "
                             + requiredContract.getName() + " with id '" + requestedId
                             + "'; available ids: " + ids(candidates)));
  }
