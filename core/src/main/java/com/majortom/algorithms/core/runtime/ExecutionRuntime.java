...
public final class ExecutionRuntime {
    ...
    public ExecutionRuntime(Clock clock, Supplier<String> runIdSupplier) {
        ...
        this.runIdSupplier = Objects.requireNonNull(runIdSupplier);
    }

    public ExecutionResult execute(String operationId, EventSink sink, ExecutionOperation<?> operation) {
    ...
    public ExecutionResult execute(
        ...
        try {
            ...
            c.checkpoint();
            Object output;
            try (ExecutionEvents.Binding ignored = ExecutionEvents.bind(c)) {
                output = operation.execute();
            }
            c.checkpoint();
            c.emitLifecycle(new RunCompletedEvent());
            return ExecutionResult.completed(output);
        } catch (ExecutionCancellationException e) {
            return cancelled(c, e.getMessage());
    ...
    private ExecutionResult cancelled(RuntimeEventContext c, String reason) {
        try {
            c.emitLifecycle(new RunCancelledEvent(reason == null ? "Execution cancelled" : reason));
        } catch (RuntimeException ignored) {
        }
