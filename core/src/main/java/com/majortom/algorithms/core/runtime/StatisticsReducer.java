...
public final class StatisticsReducer {
  ...
  public ExecutionStatistics reduce(ExecutionStatistics previous, EventEnvelope event) {
    ...
    return new ExecutionStatistics(Math.addExact(previous.totalEventCount(), 1L), domainEventCount,
        lifecycleEventCount, startedAt, endedAt, duration, metrics);
  }
  ...
  private static boolean isTerminal(EventEnvelope event) {
    return event.event() instanceof RunCompletedEvent || event.event() instanceof RunCancelledEvent
  ...
  private static Map<String, Long> mergeMetrics(Map<String, Long> previous, EventEnvelope event) {
    ...
    Map<String, Long> deltas =
        Objects.requireNonNull(contribution.metricDeltas(), "statistics contribution metricDeltas");
    if (deltas.isEmpty()) {
      return previous;
    }
