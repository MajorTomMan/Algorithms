...
class WorkbenchUiTest {
  ...
  void everyGenericSelectionFormatsBeforePublishingInBothLanguages() throws Exception {
    fx(() -> {
      ...
      for (Locale locale : List.of(Locale.CHINESE, Locale.ENGLISH)) {
        ...
        assertTrue(label("structureInspectorBody").getText().contains(text));
        invoke("showLinkedSelection",
            new LinkedListController.NodeSelection(7, value, null, 8L, 0, 2));
        assertTrue(label("structureInspectorBody").getText().contains(text));
        invoke(
            "showLinearSelection", new LinearStructureController.ItemSelection(0, value, "TOP", 2));
        assertTrue(label("structureInspectorBody").getText().contains(text));
        invoke("showGraphSelection", new GraphController.NodeSelection(7, value, 2));
        assertTrue(label("structureInspectorBody").getText().contains(text));
        invoke("showGraphSelection",
            new GraphController.EdgeSelection(9, value, VisualValue.of("终点"), true));
  ...
  void graphAutoFitRespectsSurfaceSafeInsets() throws Exception {
    ...
    fx(() -> {
      ...
      for (Node visualNode : nodes) {
        ...
        assertTrue(bounds.getMinX() >= viewport.getMinX() + insets.getLeft() - 3.0d,
            () -> "Graph node escaped left safe inset: " + bounds);
        assertTrue(bounds.getMaxX() <= viewport.getMaxX() - insets.getRight() + 3.0d,
            () -> "Graph node escaped right safe inset: " + bounds);
        assertTrue(bounds.getMinY() >= viewport.getMinY() + insets.getTop() - 3.0d,
            () -> "Graph node escaped top safe inset: " + bounds);
        assertTrue(bounds.getMaxY() <= viewport.getMaxY() - insets.getBottom() + 3.0d,
  ...
  void firstFitRemainsCenteredAfterRealPulseAcrossFamilies() throws Exception {
    for (int module : new int[] {3, 4, 5}) {
      ...
      fx(() -> {
        ...
        assertEquals(expectedCenterX, actualCenterX, 4.0d,
            () -> "Family " + module + " camera drifted horizontally after pulse");
        assertEquals(expectedCenterY, actualCenterY, 4.0d,
            () -> "Family " + module + " camera drifted vertically after pulse");
        return null;
      });
    }
  }
  ...
  void selectionAndLongValuesDoNotMoveInspectorBoundary() throws Exception {
    fx(() -> {
      ...
      for (double width : new double[] {1600, 1280, 1100, 1920}) {
        ...
        invoke("showArraySelection",
            new ArrayController.IndexSelection(0, VisualValue.of("很长的字符串".repeat(200)), 1));
  ...
  void typeControlStaysInStructureRailAndCancelPreservesData() throws Exception {
    fx(() -> {
      ...
      assertEquals(
          0, ((ComboBox<?>) node("valueTypeSelector")).getSelectionModel().getSelectedIndex());
  ...
  void genericModulesCanLeaveAndReturnAfterTypeChange() throws Exception {
    for (int module = 0; module < 6; module++) {
      ...
      fx(() -> {
        ...
        assertEquals(String.class, ((RuntimeValueTypeSupport) subController()).runtimeValueType());
        assertEquals(
            1, ((ComboBox<?>) node("valueTypeSelector")).getSelectionModel().getSelectedIndex());
        assertFalse(((javafx.scene.layout.VBox) node("structureKindHost")).getChildren().isEmpty());
        assertFalse(
            ((javafx.scene.layout.VBox) node("structureControlsHost")).getChildren().isEmpty());
        if (selectedModule == 4) {
        ...
        assertEquals(I18N.text(selectedModule == 4 ? "label.value_type.node"
                             : selectedModule == 5 ? "label.value_type.vertex"
        ...
        ((BaseModuleController<?>) subController())
            .applyBulkData(selectedModule == 5 ? "alpha,beta | alpha->beta:5" : "alpha,beta");
        assertTrue(((RuntimeValueTypeSupport) subController()).hasValues());
        answerNextTypeDialog(true);
        ((ComboBox<?>) node("valueTypeSelector")).getSelectionModel().select(0);
        moduleButton(7).fire();
  ...
  void familyNavigatorGeometryIsStableAcrossSelectionAndMode() throws Exception {
    fx(() -> {
      ...
      moduleButton(5).fire();
      layout();
      assertFamilyGeometryEquals(baseline, familyGeometry(navigator),
          "Selecting Graph must not change family navigation geometry");

      ((Button) node("algorithmWorkspaceBtn")).fire();
      layout();
      assertSame(navigator, node("familyNavigator"),
      ...
      assertFamilyGeometryEquals(
          baseline, familyGeometry(navigator), "Selecting Tree must be presentation-only");
    ...
    for (double width : new double[] {1600.0d, 1200.0d, 1000.0d}) {
      for (double size : new double[] {16.25d, 18.0d, 20.0d, 22.0d, 24.0d}) {
        fx(() -> {
          ...
          double nameX =
              ((Label) navigation.items().getFirst().lookup(".family-item-name")).getLayoutX();
          double previousY = -1.0d;
          for (FamilyItemView item : navigation.items()) {
            assertEquals(
                rowHeight, item.getHeight(), 0.5d, "all family rows must have identical height");
            Label name = (Label) item.lookup(".family-item-name");
            assertNotNull(name);
            assertEquals(nameX, name.getLayoutX(), 0.5d, "family name column must stay aligned");
            assertTrue(name.getWidth() + 0.5d >= name.prefWidth(-1.0d),
                ()
                    -> "family label clipped at " + size + "px / " + width + "px: " + name.getText()
                    + " width=" + name.getWidth() + " pref=" + name.prefWidth(-1.0d));
            assertTrue(item.getLayoutY() > previousY, "family rows must keep registry order");
  ...
  private List<FamilyGeometry> familyGeometry(FamilyNavigator navigator) {
    return navigator.items()
        .stream()
        .map(item
            -> new FamilyGeometry(item.familyId(), item.getLayoutX(), item.getLayoutY(),
  ...
  private record
      FamilyGeometry(String id, double x, double y, double width, double height, double nameX) {}
  ...
  void failedModulePreparationPreservesCurrentControlsDataAndNavigation() throws Exception {
    fx(() -> {
      ...
      var controls =
          List.copyOf(((javafx.scene.layout.VBox) node("structureControlsHost")).getChildren());
      var broken = new com.majortom.algorithms.visualization.module.WorkbenchModuleDefinition(
          "graph", "Graph",
          () -> new BaseModuleController<Object>(null, "/fxml/missing-test-panel.fxml") {
            @Override
      ...
      assertEquals(before, snapshotState());
      assertEquals(
          controls, ((javafx.scene.layout.VBox) node("structureControlsHost")).getChildren());
      assertTrue(moduleButton(0).getPseudoClassStates().contains(
  ...
  private void answerNextTypeDialog(boolean confirm) {
    Platform.runLater(() -> {
      for (Window window : List.copyOf(Window.getWindows())) {
        if (window == stage || window.getScene() == null)
          continue;
        Node pane = window.getScene().getRoot();
        if (pane instanceof DialogPane dialog) {
          ButtonType answer = dialog.getButtonTypes()
                                  .stream()
                                  .filter(b -> b.getButtonData().isCancelButton() != confirm)
                                  .findFirst()
                                  .orElseThrow();
  ...
  private static void click(Node node) {
    assertNotNull(node);
    node.fireEvent(new MouseEvent(MouseEvent.MOUSE_CLICKED, 10, 10, 10, 10, MouseButton.PRIMARY, 1,
