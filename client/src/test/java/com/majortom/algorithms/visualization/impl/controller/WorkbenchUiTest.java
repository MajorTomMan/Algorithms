...
class WorkbenchUiTest {
    ...
    @Test void familyRailKeepsFullLabelsAcrossFontAndViewportSweep() throws Exception {
            ...
            for (double size : new double[]{16.25d, 18.0d, 20.0d, 21.0d, 22.0d, 24.0d}) {
                fx(() -> {
                    ...
                    for (Node item : navigation.getChildren()) {
                        ...
                        assertTrue(button.getWidth() + 0.5d >= button.prefWidth(-1.0d),
                                () -> "family label clipped at " + size + "px / " + width
                                        + "px: " + button.getText() + " width=" + button.getWidth()
                                        + " pref=" + button.prefWidth(-1.0d));
                    }
                    return null;
                });
            }
