...
public interface ClientExecutionService extends AutoCloseable {
  <S> ExecutionHandle start(String operationId, ExecutionOperation<?> operation,
  ...
  default<S> ExecutionHandle start(String operationId, ExecutionOperation<?> operation,
      EventReducer<S> reducer, Consumer<S> liveStateConsumer,
    ...
    return start(operationId, operation, reducer,
        ignored -> {}, liveStateConsumer, liveStatisticsConsumer, delayMillisSupplier);
  }
  ...
  static <S> void requireStartArguments(String operationId, ExecutionOperation<?> operation,
      EventReducer<S> reducer, Consumer<EventEnvelope> liveEventConsumer,
      Consumer<S> liveStateConsumer, Consumer<ExecutionStatistics> liveStatisticsConsumer,
      LongSupplier delayMillisSupplier) {
