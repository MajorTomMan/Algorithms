...
public final class RenderSurfaceHost {
  private final RenderSurfaceLifecycle lifecycle;
  private final Map<RenderSessionId, CompletionStage<Void>> lifecycleTails = new HashMap<>();

  public RenderSurfaceHost(RenderSurfaceLifecycle lifecycle) {
    this.lifecycle = Objects.requireNonNull(lifecycle, "lifecycle");
  }

  public CompletionStage<Void> attach(RenderSurface<?> surface) {
    Objects.requireNonNull(surface, "surface");
    return serialized(surface.sessionId(), () -> lifecycle.register(surface)
        .thenCompose(ignored -> lifecycle.activate(surface.sessionId())));
  }
  ...
  public CompletionStage<Void> detach(RenderSurface<?> surface, Runnable dispose) {
    ...
    return serialized(surface.sessionId(), () -> lifecycle.deactivate(surface.sessionId())
        .thenCompose(ignored -> lifecycle.unregister(surface))
        .thenCompose(ignored -> lifecycle.disposeOnFx(dispose)));
  }
  ...
  private synchronized CompletionStage<Void> serialized(
    ...
    CompletionStage<Void> next = previous.handle((ignored, failure) -> null)
