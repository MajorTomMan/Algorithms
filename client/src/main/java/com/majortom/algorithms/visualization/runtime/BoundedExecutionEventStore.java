...
final class BoundedExecutionEventStore implements EventSink {
  ...
  synchronized EventEnvelope recordLimitFailure() {
    ...
    if (!events.isEmpty() && events.getLast().event() instanceof RunFailedEvent failedEvent
        && LIMIT_FAILURE_CODE.equals(failedEvent.code())) {
      return events.getLast();
    }
    String message = limitMessage();
    EventEnvelope failedEvent = new EventEnvelope(rejectedEvent.runId(),
        rejectedEvent.operationId(), rejectedEvent.sequence(), rejectedEvent.timestamp(),
        rejectedEvent.source(), new RunFailedEvent(LIMIT_FAILURE_CODE, message));
