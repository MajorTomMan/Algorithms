...
final class WorkbenchTheme {
  ...
  private static void showSavedSnapshotMenu(Button source) {
    ...
    if (source.getFont() != null) {
      menu.setStyle(
          String.format(java.util.Locale.ROOT, "-fx-font-size: %.2fpx; -fx-font-family: \"%s\";",
  ...
  private static void applyButtonSemantic(Node node) {
    if (hasAny(node, "btn-primary", "btn-run-neon", "btn-ran-blue", "btn-ran-purple",
            "btn-neon-cyan")) {
      ...
      add(node, Styles.WARNING);
                   ...
                   "module-button", "sidebar-catalog-button", "sidebar-algorithm-button",
