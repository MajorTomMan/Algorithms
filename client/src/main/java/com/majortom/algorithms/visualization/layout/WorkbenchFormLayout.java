...
public final class WorkbenchFormLayout {
  ...
  private static void installSectionMetricPolicy(VBox section) {
    ...
    section.sceneProperty().addListener(
        (observable, oldScene, newScene) -> scheduleMetricRefresh(section));
  ...
  private static void installFontListeners(VBox section, Node node) {
    if (node instanceof TextInputControl input && isOperationControl(input)) {
      input.fontProperty().addListener(
      ...
      labeled.fontProperty().addListener(
          (observable, oldValue, newValue) -> scheduleMetricRefresh(section));
    }
    if (node instanceof Parent parent) {
      for (Node child : parent.getChildrenUnmodifiable()) {
  ...
  private static double metricHeight(Region control) {
    ...
    Text probe = new Text("国Ag");
    probe.setFont(font);
    return probe.getLayoutBounds().getHeight() + control.getInsets().getTop()
        + control.getInsets().getBottom() + METRIC_SAFETY_PADDING;
  }
  ...
  private enum OperationRole { NONE, INPUT, BUTTON }
}
