...
public final class DefaultRenderFramework implements RenderPort, AutoCloseable {
  ...
  public DefaultRenderFramework(RenderScheduler scheduler, LayoutExecutor layoutExecutor,
      FxExecutor fxExecutor, FxSurfaceRegistry surfaces, LayoutEngineRegistry layoutEngines,
      CameraManager cameraManager, RenderTrace trace) {
  ...
  public <S> CompletionStage<Void> registerSurface(
    ...
    return fxExecutor.execute(() -> {
      ...
      adapter.setViewportListener(
          viewport -> submit(new ViewportRenderIntent(id, viewport, CameraPolicy.KEEP)));
    });
  }

  public CompletionStage<Void> unregisterSurface(RenderSessionId id, FxSurfaceAdapter<?> adapter) {
  ...
  public CompletionStage<RenderResult> submit(RenderIntent intent) {
    ...
    scheduler.execute(() -> {
      ...
      RenderMailbox mailbox =
          mailboxes.computeIfAbsent(intent.sessionId(), ignored -> new RenderMailbox());
      ...
      mailbox.enqueue(
          new RenderMailbox.Submission(intent, future, modelRevision, geometryRevision));
      if (session.active())
        drain(intent.sessionId());
    });
    return future;
  }
  ...
  public CompletionStage<Void> updateContentStyle(ContentStyleSnapshot next) {
    ...
    scheduler.execute(() -> {
      ...
      for (RenderSession session : sessions.values()) {
        if (!session.active() || session.latestStructuralSnapshot == null)
          continue;
        refreshes.add(
            submit(new StructuralRenderIntent<>(session.id, session.latestStructuralSnapshot,
                CameraPolicy.ENSURE_VISIBLE, false, StructuralChange.GEOMETRY)));
      }
  ...
  private void drain(RenderSessionId sessionId) {
    ...
    if (!session.active() || mailbox.inFlight)
      return;
    ...
    stage.whenComplete((result, failure) -> scheduler.execute(() -> {
      ...
      if (failure != null) {
        submission.future().complete(new RenderResult(RenderStatus.FAILED, sessionId,
            session.committedRevision, false,
  ...
  private <S> CompletionStage<RenderResult> processStructural(RenderSession session,
      StructuralRenderIntent<S> intent, long modelRevision, long geometryRevision) {
    long generation = session.generation;
    long transactionId = transactionSequence.incrementAndGet();
    RenderTransaction transaction = new RenderTransaction(transactionId, session.id, intent.kind(),
        generation, modelRevision, geometryRevision, System.nanoTime());
    ...
    RenderCaptureContext captureContext = new RenderCaptureContext(transactionId, session.id,
        generation, modelRevision, geometryRevision,
        new com.majortom.algorithms.visualization.render.api.LayoutRequestId(
            requestSequence.incrementAndGet()),
        intent.change(), contentStyle, intent.initialFrame());

    return prepareStage
        .thenCompose(ignored
            -> fxExecutor.supply(()
                                     -> surfaces.<S>require(session.id)
                                         .captureLayout(intent.snapshot(), captureContext)))
        .thenCompose(request
            -> onScheduler(
                () -> continueStructuralAfterCapture(session, transaction, intent, request)))
        .thenCompose(stage -> stage);
  }

  private <S> CompletionStage<RenderResult> continueStructuralAfterCapture(RenderSession session,
      RenderTransaction transaction, StructuralRenderIntent<S> intent, LayoutRequest request) {
    if (!authoritative(session, transaction, intent.change()))
      return completedCancelled(session);
    ...
    if (layoutRequired) {
      ...
      layoutStage = CompletableFuture.completedFuture(
          session.layout.withModelRevision(transaction.modelRevision()));
    }

    return layoutStage
        .thenCompose(layoutResult
            -> onScheduler(()
                               -> continueStructuralAfterLayout(session, transaction, intent,
                                   request, layoutResult, layoutRequired)))
        .thenCompose(stage -> stage);
  }

  private <S> CompletionStage<RenderResult> continueStructuralAfterLayout(RenderSession session,
      RenderTransaction transaction, StructuralRenderIntent<S> intent, LayoutRequest request,
      LayoutResult layoutResult, boolean layoutChanged) {
    if (!authoritative(session, transaction, intent.change()))
      return completedCancelled(session);
    ...
    RenderCommitContext commitContext = RenderCommitContext.structural(transaction.id(), session.id,
        session.generation, transaction.modelRevision(), nextLayoutRevision,
        session.presentationRevision, intent.change(), intent.initialFrame());
    ...
    return fxExecutor
        ...
        .thenCompose(ignored
            -> onScheduler(()
                               -> continueStructuralAfterApply(session, transaction, intent,
                                   request, layoutResult, layoutChanged)))
        .thenCompose(stage -> stage);
  }

  private <S> CompletionStage<RenderResult> continueStructuralAfterApply(RenderSession session,
      RenderTransaction transaction, StructuralRenderIntent<S> intent, LayoutRequest request,
      LayoutResult layoutResult, boolean layoutChanged) {
    if (!authoritative(session, transaction, intent.change()))
      return completedCancelled(session);
    ...
    return fxExecutor
        .supply(() -> {
          ...
          CameraState resolved = cameraManager.resolve(effectivePolicy, layoutResult.bounds(),
              viewport, current, session.camera, target.userControlledCamera(), MIN_CAMERA_SCALE,
              MAX_AUTO_FIT_SCALE);
          if (!cameraClose(resolved, current))
            target.applyCameraState(resolved);
        ...
        .thenCompose(
            expectedCamera -> pulseBarrier.await().thenCompose(ignored -> fxExecutor.supply(() -> {
              FxSurfaceAdapter<S> target = surfaces.require(session.id);
  ...
  private <S> CompletionStage<RenderResult> processPresentation(
    ...
    long modelRevision = session.modelRevision;
    RenderTransaction transaction = new RenderTransaction(transactionId, session.id, intent.kind(),
        generation, modelRevision, session.geometryRevision, System.nanoTime());
    long presentationRevision = ++session.presentationRevision;
    RenderCommitContext context = RenderCommitContext.presentation(transactionId, session.id,
        generation, modelRevision, session.layoutRevision, presentationRevision);
    trace(transaction, RenderPipeline.WAIT_APPLY);
    return fxExecutor
        .supply(
            () -> surfaces.<S>require(session.id).commitPresentation(intent.snapshot(), context))
        .thenCompose(stage -> stage)
  ...
  private CompletionStage<RenderResult> processViewport(
    ...
    long transactionId = transactionSequence.incrementAndGet();
    RenderTransaction transaction = new RenderTransaction(transactionId, session.id, intent.kind(),
        generation, session.modelRevision, session.geometryRevision, System.nanoTime());
    if (session.layout == null) {
      return CompletableFuture.completedFuture(new RenderResult(RenderStatus.NO_OP, session.id,
          session.modelRevision, false, BoundsSnapshot.empty(), null));
    }
      ...
      return fxExecutor.supply(() -> surfaces.<Object>require(session.id).cameraState())
          .thenCompose(camera -> onScheduler(() -> {
            if (session.generation != generation || !session.active())
              return RenderResult.cancelled(session.id);
            session.camera = camera;
            return new RenderResult(RenderStatus.NO_OP, session.id, session.modelRevision, false,
                session.layout.bounds(), null);
          }));
    }
    return fxExecutor
        .supply(() -> {
          ...
          CameraState current = target.cameraState();
          CameraState resolved = cameraManager.resolve(intent.cameraPolicy(),
              session.layout.bounds(), intent.viewport(), current, session.camera,
        ...
        .thenCompose(camera -> onScheduler(() -> {
          if (session.generation != generation || !session.active())
            return RenderResult.cancelled(session.id);
          session.camera = camera;
          return new RenderResult(RenderStatus.PRESENTED, session.id, session.modelRevision, false,
              session.layout.bounds(), null);
        }));
  }
  ...
  private boolean authoritative(
    ...
    if (!session.active() || session.generation != transaction.generation()
  ...
  private void trace(RenderTransaction transaction, RenderPipeline stage) {
    trace.stage(transaction.sessionId(), transaction.id(), stage.name(), transaction.generation(),
