 ...
 *
 * <p>Code outside this package never calls {@code FxDispatch.defer()} directly. RenderFramework and
 * ordinary UI shell code share the same JavaFX entry point without sharing render-session
 * authority.
 */
    ...
    private static void observe(CompletionStage<Void> stage) {
        stage.whenComplete(
                (ignored, failure) -> {
                    ...
                    if (handler != null) handler.uncaughtException(thread, cause);
                });
    }
}
