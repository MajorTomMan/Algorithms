...
public final class PlaybackToolbar extends Pane {
  ...
  protected double computePrefWidth(double height) {
    ...
    return snappedLeftInset() + snappedRightInset() + width
  ...
  protected void layoutChildren() {
    ...
    for (Placement placement : plan.placements()) {
      ...
      child.resizeRelocate(snapPositionX(left + placement.x()), snapPositionY(top + placement.y()),
          snapSizeX(placement.width()), snapSizeY(placement.height()));
    }
  }
  ...
  private LayoutPlan oneRow(List<Node> children, Node timeline, double width) {
    ...
    double fixedWidth =
        children.stream().filter(node -> node != timeline).mapToDouble(this::prefWidth).sum();
    ...
    double rowHeight =
        children.stream()
            .mapToDouble(
                node -> prefHeight(node, node == timeline ? timelineWidth : prefWidth(node)))
            .max()
            .orElse(0.0d);
    ...
    for (Node child : children) {
      ...
      placements.add(new Placement(
          child, x, Math.max(0.0d, (rowHeight - childHeight) / 2.0d), childWidth, childHeight));
  ...
  private LayoutPlan wrapped(List<Node> children, Node timeline, double width) {
    ...
    List<Placement> placements = new ArrayList<>();
    double topHeight =
        topRow.stream().mapToDouble(node -> prefHeight(node, prefWidth(node))).max().orElse(0.0d);
    ...
    if (topRequired <= width) {
      for (Node child : topRow) {
        ...
        double childHeight = prefHeight(child, childWidth);
        placements.add(new Placement(
            child, x, Math.max(0.0d, (topHeight - childHeight) / 2.0d), childWidth, childHeight));
        x += childWidth + horizontalGap;
      }
    } else {
      ...
      double controlsHeight = controls.stream()
      ...
      for (Node child : controls) {
        ...
        double childHeight = prefHeight(child, childWidth);
        placements.add(new Placement(child, x,
            Math.max(0.0d, (controlsHeight - childHeight) / 2.0d), childWidth, childHeight));
        x += childWidth + horizontalGap;
      }
    ...
    double speedWidth =
        topRow.size() == 1 && isSpeed(topRow.getFirst()) ? prefWidth(topRow.getFirst()) : 0.0d;
    ...
    if (speedWidth > 0.0d) {
      ...
      placements.add(new Placement(speed, timelineWidth + horizontalGap,
          secondY + Math.max(0.0d, (timelineHeight - speedHeight) / 2.0d), speedWidth,
          speedHeight));
