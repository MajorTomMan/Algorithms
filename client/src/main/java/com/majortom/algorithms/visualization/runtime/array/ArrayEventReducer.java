...
public final class ArrayEventReducer implements EventReducer<ArrayViewState> {
    ...
    public ArrayEventReducer(List<?> initialValues) {
    ...
    public Reduction<ArrayViewState> reduce(ArrayViewState previous, EventEnvelope envelope) {
        ...
        if (event instanceof ArrayStructureEvent.Swapped swapped) {
            ...
            return changed(
                    state(
                            values,
                            ArrayViewState.Mutation.swapped(
        ...
        if (event instanceof RunCompletedEvent) {
            return Reduction.changed(
                    new ArrayViewState(
                            previous.values(),
                            ArrayViewState.Mutation.none(),
                            ArrayViewState.Observation.none(),
                            true),
    ...
    private static ArrayViewState state(
            List<VisualValue> values, ArrayViewState.Mutation mutation) {
    ...
    private static ArrayViewState.Observation observation(ObservationEvent.Compared compared) {
        ...
        if (leftIndex != null
                && compared.rightRef() instanceof ObservationEvent.ValueRef valueRef) {
            return ArrayViewState.Observation.comparedValue(leftIndex, valueRef.value());
        }
        if (rightIndex != null
                && compared.leftRef() instanceof ObservationEvent.ValueRef valueRef) {
    ...
    private static Integer arrayIndex(ObservationEvent.Reference reference) {
        if (reference instanceof ObservationEvent.IndexRef indexRef
                && ARRAY_SOURCE.equals(indexRef.source())) return indexRef.index();
        return null;
    }
