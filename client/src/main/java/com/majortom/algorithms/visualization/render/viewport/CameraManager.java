...
public final class CameraManager {
  public CameraState resolve(CameraPolicy policy, BoundsSnapshot content, ViewportSnapshot viewport,
      CameraState current, CameraState restored, boolean userControlled, double minScale,
      double maxScale) {
    if (policy == CameraPolicy.RESTORE && restored != null)
      return restored;
    if (policy == CameraPolicy.KEEP || content == null || content.isEmpty())
      return current;
    if (userControlled)
      return current;
