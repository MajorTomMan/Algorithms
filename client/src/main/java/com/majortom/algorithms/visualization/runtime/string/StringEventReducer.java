...
public final class StringEventReducer implements EventReducer<StringViewState> {
    ...
    public Reduction<StringViewState> reduce(StringViewState previous, EventEnvelope envelope) {
        ...
        if (event instanceof StringStructureEvent.Inserted inserted) {
            ...
            return changed(
                    state(
                            value,
                            StringViewState.Mutation.inserted(
        ...
        if (event instanceof StringStructureEvent.Removed removed) {
            ...
            return changed(
                    state(
                            value,
                            StringViewState.Mutation.removed(
                                    removed.index(), removed.value().length()),
                            previous.patternStart()));
        }
        ...
        if (event instanceof StringStructureEvent.Replaced replaced) {
            String value =
                    replace(
                            previous.value(),
                            replaced.index(),
                            replaced.previousValue().length(),
                            replaced.value());
            return changed(
                    state(
                            value,
                            StringViewState.Mutation.replaced(
        ...
        if (event instanceof ObservationEvent.Compared compared) {
            ...
            if (targetIndex != null && patternIndex != null) {
                ...
                return observation(
                        new StringViewState(
                                previous.value(),
                                StringViewState.Mutation.none(),
                                StringViewState.Observation.compared(targetIndex, patternIndex),
                                patternStart,
                                false));
            }
        }
        if (event instanceof ObservationEvent.Matched matched) {
            return observation(
                    new StringViewState(
                            previous.value(),
                            StringViewState.Mutation.none(),
                            StringViewState.Observation.matched(matched.index(), matched.length()),
                            matched.index(),
                            false));
        }
        if (event instanceof ObservationEvent.Fallback fallback) {
            ...
            return observation(
                    new StringViewState(
                            previous.value(),
                            StringViewState.Mutation.none(),
                            StringViewState.Observation.fallback(
                                    fallback.fromIndex(), fallback.toIndex()),
                            patternStart,
                            false));
        }
    ...
    private static StringViewState state(
            String value, StringViewState.Mutation mutation, int patternStart) {
        return new StringViewState(
