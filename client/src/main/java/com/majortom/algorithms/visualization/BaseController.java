...
public abstract class BaseController<S> implements Initializable {
  ...
  private static final ExecutionExportCodec DEFAULT_EXPORT_CODEC =
      new ExecutionExportCodec(JSON_MAPPER);
  private static final ExecutionExporter DEFAULT_EXECUTION_EXPORTER = new JsonExecutionExporter(
      java.nio.file.Path.of("exports"), JSON_MAPPER, DEFAULT_EXPORT_CODEC);
  ...
  private final ChangeListener<Number> timelineSliderListener =
    ...
    if (!updatingTimelineSlider && !running.get() && timelineSlider != null
        && timelineSlider.isValueChanging()) {
  ...
  protected BaseController(BaseVisualizer<S> visualizer) {
    this(visualizer, new LocalClientExecutionService(), DEFAULT_EXECUTION_HISTORY,
        DEFAULT_INPUT_FINGERPRINT, DEFAULT_EXECUTION_EXPORTER);
  }

  protected BaseController(BaseVisualizer<S> visualizer, ClientExecutionService execution,
      RunHistoryService executionHistory, InputFingerprint inputFingerprint,
  ...
  protected final void startAlgorithm(String algorithmId, Object input,
      ExecutionOperation<?> operation, Supplier<? extends EventReducer<S>> reducerFactory) {
    if (disposed) {
    ...
    currentSession = execution.start(algorithmId, operation, liveReducer, this::consumeLiveEvent,
        this::renderLiveState, this::updateLiveStatistics, livePlaybackDelayMillis::get);
    ExecutionHandle session = currentSession;
    session.presentationCompletion().whenComplete(
        (result, error)
            -> FxDispatch.defer(
  ...
  /**
   * Executes one editable structure mutation through the shared Runtime and records its event
   * history.
   */
  protected final boolean executeStructureOperation(
      String operationId, ExecutionOperation<?> operation) {
    if (disposed) {
      throw new IllegalStateException("Controller is disposed");
    }
    ...
    String message =
        result.failure().map(failure -> failure.message()).orElse("Structure operation failed");
    appendLog(message);
    return false;
  }
  ...
  private void finishExecution(ExecutionHandle session, String algorithmId, Object input,
      Supplier<? extends EventReducer<S>> reducerFactory, ExecutionResult result, Throwable error) {
    ...
    ExecutionSummary summary =
        ExecutionSummary.from(stats, session.resourceUsage())
            .withTiming(ExecutionTiming.of(eventSpan, session.totalDuration()));
    lastExecution =
        createExecutionRecord(algorithmId, input, result, error, summary, events, timeline.size());
  ...
  /**
   * Modules with selectable saved-snapshot input override this to preserve independent algorithm
   * state.
   */
  ...
  /**
   * Applies a replay seek without allowing reducer or renderer failures to escape JavaFX
   * callbacks.
   */
  ...
  private String describeRecord(ClientExecutionRecord record) {
    ...
    return String.format("%s | event-span=%dms | total=%s | cpu=%s | memory=%s | events=%d | "
                         + "frames=%d | compares=%d",
        record.operationId(), timing.eventSpan().toMillis(), formatDuration(timing.totalDuration()),
        formatNanos(summary.resources().cpuTimeNanos()),
        formatBytes(summary.resources().peakMemoryBytes()),
        record.recording().statistics().totalEventCount(), record.visualFrameCount(),
  ...
  private void updatePlaybackButtonState() {
    ...
    if (pauseBtn != null) {
      boolean replayActive = !running.get() && replayController != null
  ...
  protected void onAlgorithmFinished(ExecutionResult result) {
    appendLog(String.format(
        "Finished. Event span: %dms", executionSummary().timing().eventSpan().toMillis()));
  }
  ...
  public final String latestRunId() {
    ...
    if (!events.isEmpty())
      return events.getFirst().runId();
    if (lastExecution != null)
      return lastExecution.recording().runId();
    return null;
  }
  ...
  public final String latestResultText() {
    if (lastExecution == null)
      return I18N.text("label.workspace.result.none");
  ...
  private ClientExecutionRecord createExecutionRecord(String operationId, Object input,
      ExecutionResult result, Throwable error, ExecutionSummary summary, List<EventEnvelope> events,
    ...
    ExecutionSummary recordingSummary =
        ExecutionSummary.from(authoritativeStatistics, summary.resources())
            .withTiming(ExecutionTiming.of(
                authoritativeStatistics.eventSpan(), summary.timing().totalDuration()));
    ...
    if (effectiveResult == null) {
      ...
      effectiveResult =
          ExecutionResult.failed(new com.majortom.algorithms.core.runtime.ExecutionFailure(
              "client.execution.failed", message, exceptionType));
    }
    return new ClientExecutionRecord(moduleId(), operationId, inputFingerprint(input),
        effectiveResult, recording, visualFrameCount);
  }

  private boolean hasTerminalLifecycleEvent(List<EventEnvelope> events) {
    for (EventEnvelope event : events) {
      if (event.event() instanceof RunCompletedEvent || event.event() instanceof RunCancelledEvent
