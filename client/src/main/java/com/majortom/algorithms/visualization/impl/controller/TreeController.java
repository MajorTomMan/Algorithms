...
public final class TreeController extends BaseModuleController<TreeViewState>
  ...
  @SuppressWarnings("rawtypes") private AVLTree avlTree;
  ...
  public record
      NodeSelection(long id, VisualValue value, Long parentId, int childCount, int depth) {}

  private enum TreeVariant { GENERAL, AVL }
  ...
  private boolean publishAlgorithmSelection(TreeViewState state, long nodeId) {
    ...
    selectionListener.accept(
        new NodeSelection(nodeId, node.value(), parentId, state.childrenOf(node).size(), depth));
    return true;
  }
  ...
  private void handleGeneralSelection(long nodeId) {
    ...
    selectionListener.accept(new NodeSelection(nodeId, VisualValue.of(node.getValue()), parentId,
        node.getChildren().size(), generalDepthOf(generalTree.root(), node, 0)));
  ...
  private void handleAvlSelection(long nodeId) {
    ...
    selectionListener.accept(new NodeSelection(nodeId, VisualValue.of(node.getValue()), parentId,
        childCount, avlDepthOf(avlRoot(), node, 0)));
    refreshOperationAvailability();
  }
  ...
  public void handleAlgorithmStart() {
    ...
    AlgorithmDescriptor descriptor =
        AlgorithmCatalog.compatibleDescriptor(activeStructure, runtimeValueType, algorithmId);
  ...
  private BinaryTreeSnapshot.Node<Object> snapshotBinaryNode(AVLTreeNode<Object> node) {
    ...
    return new BinaryTreeSnapshot.Node<>(node.getId(), node.getValue(),
        snapshotBinaryNode(left(node)), snapshotBinaryNode(right(node)));
  }
  ...
  private int presentationDepth(
      TreeViewState state, Long currentId, long targetId, int depth, java.util.Set<Long> visited) {
  ...
  private void refreshAlgorithmIds() {
    if (activeVariant == TreeVariant.GENERAL) {
      algorithmIds = AlgorithmCatalog.compatibleAlgorithms(
          com.majortom.algorithms.structure.tree.GeneralTreeStructure.class, runtimeValueType);
    } else {
      algorithmIds = AlgorithmCatalog.compatibleAlgorithms(
          com.majortom.algorithms.structure.tree.AvlTreeStructure.class, runtimeValueType);
    }
  }
  ...
  public String structureSummaryText() {
    if (activeVariant == TreeVariant.GENERAL) {
      ...
      return I18N.text("label.workspace.structure.summary.tree", snapshot.size(),
          generalHeight(snapshot.root()), rootValue);
    }
    ...
    return I18N.text("label.workspace.structure.summary.tree", snapshot.size(),
        binaryHeight(snapshot.root()), rootValue);
  }
  ...
  protected String formatStatsMessage() {
    ...
    return String.format("%s | %s | %s", I18N.text("stats.size", size),
        I18N.text("stats.height", height),
        formatMetric(
            "stats.action", stats.metric("nodes.inserted") + stats.metric("nodes.removed")));
  }
