...
public final class MazeVisualizer
    extends BaseVisualizer<MazeViewState> implements FxSurfaceAdapter<MazeViewState> {
  ...
  public LayoutRequest captureLayout(MazeViewState state, RenderCaptureContext context) {
    ...
    if (state.rows() < 1 || state.columns() < 1) {
      ...
      elements = List.of(new LayoutElement(
          GRID_ID, state.columns() * WORLD_CELL_SIZE, state.rows() * WORLD_CELL_SIZE));
    }
  ...
  private void drawBaseGrid(MazeViewState state, double cellWidth, double cellHeight) {
    ...
    for (int row = 0; row < state.rows(); row++) {
      for (int column = 0; column < state.columns(); column++) {
        ...
        gc.strokeRect(x + 0.5d, y + 0.5d, Math.max(0.0d, cellWidth - 1.0d),
  ...
  private void drawGraphEdges(MazeViewState state, double cellWidth, double cellHeight) {
    ...
    for (MazeSnapshot.Edge edge : state.graphEdges()) {
      ...
      gc.strokeLine((fromColumn + 0.5d) * cellWidth, (fromRow + 0.5d) * cellHeight,
          (toColumn + 0.5d) * cellWidth, (toRow + 0.5d) * cellHeight);
    }
    gc.restore();
  }
  ...
  private void drawVisited(MazeViewState state, double cellWidth, double cellHeight) {
    ...
    for (GridPoint point : state.visited()) {
      ...
      gc.fillRect(x + inset, y + inset, Math.max(0.0d, cellWidth - inset * 2.0d),
  ...
  private void drawObserved(MazeViewState state, double cellWidth, double cellHeight) {
    ...
    gc.fillRect(point.column() * cellWidth + inset, point.row() * cellHeight + inset,
        Math.max(0.0d, cellWidth - inset * 2.0d), Math.max(0.0d, cellHeight - inset * 2.0d));
  }
  ...
  private void drawRole(GridPoint point, String label, Color stroke, MazeViewState state,
    ...
    gc.strokeRect(x + inset, y + inset, Math.max(0.0d, cellWidth - inset * 2.0d),
        Math.max(0.0d, cellHeight - inset * 2.0d));
    if (density != VisualDensity.DENSE && Math.min(cellWidth, cellHeight) >= 10.0d) {
      gc.setFill(RAN_BLACK);
      gc.setTextAlign(TextAlignment.CENTER);
      gc.setFont(Font.font(
          "Consolas", FontWeight.BOLD, Math.max(8.0d, Math.min(cellWidth, cellHeight) * 0.46d)));
  ...
  private VisualDensity densityFor(double cellSize) {
    if (cellSize < DENSE_CELL_THRESHOLD)
      return VisualDensity.DENSE;
    if (cellSize < COMPACT_CELL_THRESHOLD)
      return VisualDensity.COMPACT;
    return VisualDensity.DETAIL;
  }

  private static boolean inside(MazeViewState state, GridPoint point) {
    return point != null && point.row() >= 0 && point.row() < state.rows() && point.column() >= 0
