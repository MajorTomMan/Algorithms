...
public final class MazeVisualizer extends CanvasVisualizer<MazeViewState> implements RenderSurface<MazeViewState> {
    ...
    private void paint(MazeViewState state) {
        ...
        double cellWidth = canvas.getWidth() / state.columns();
        double cellHeight = canvas.getHeight() / state.rows();
        double screenCellSize = Math.min(cellWidth, cellHeight) * Math.max(0.01d, surface.cameraState().scale());
