...
public class SnakeView {
  ...
  private void renderWelcome(StringBuilder sb) {
    appendPanel(sb, "贪 吃 蛇", "一个运行在终端里的小游戏", "", "操作说明",
        "W A S D 或方向键  -> 每次移动一格", "P                -> 游戏中暂停/继续",
        "Q                -> 退出游戏", "", "按 Enter、空格或 S 开始");
    sb.append("\n");
    appendPanel(sb, "小提示", "这是一步一动的回合制版本。", "每次输入只会让蛇前进一步。",
        "规划好路线，别撞墙，也别咬到自己。");
  }
  ...
  private void renderPlaying(StringBuilder sb, Snake snake, Food food, Map map, boolean paused) {
    ...
    appendPanel(sb, title, "分数   : " + score, "长度   : " + snake.length(),
        "蛇头坐标: (" + head.x + ", " + head.y + ")", "食物坐标: (" + food.x + ", " + food.y + ")",
        help);
    sb.append("\n");
  ...
  private void renderGameOver(StringBuilder sb, Snake snake, Food food, Map map) {
    Point head = snake.getHead();
    int score = Math.max(0, snake.length() - 1);
    appendPanel(sb, "游 戏 结 束", "分数     : " + score, "长度     : " + snake.length(),
        "最终蛇头坐标: (" + head.x + ", " + head.y + ")", "按 R 重新开始，按 Q 退出");
    sb.append("\n");
    appendBoard(sb, snake, food, map);
  }
