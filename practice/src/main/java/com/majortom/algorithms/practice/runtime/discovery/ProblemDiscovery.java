...
public final class ProblemDiscovery {
  ...
  public List<ProblemDescriptor> discover(ClassLoader classLoader) {
    ...
    for (Class<?> candidate : candidates) {
      ...
      descriptors.add(new ProblemDescriptor(problem.source(), problem.id(),
          ComponentNames.resolve(problem.name(), candidate), problem.number(), problem.difficulty(),
          Arrays.asList(problem.tags()), candidate, entry));
    }
